...
describe("createApplicationSchema", () => {
  ...
  it("rejects invalid UUID, enums, dates, and URLs", () => {
    expect(
      createApplicationSchema.safeParse({ requestId: "nope", company: "IBM", title: "X" }).success,
    ).toBe(false);
    expect(
  ...
  it("requires company and title", () => {
    expect(
      createApplicationSchema.safeParse({ requestId: REQ, company: " ", title: "X" }).success,
    ).toBe(false);
    expect(createApplicationSchema.safeParse({ requestId: REQ, company: "IBM" }).success).toBe(
      false,
    );
  });
});
...
describe("updateApplicationStatusSchema", () => {
  ...
  it("keeps explicit null appliedAt distinct from omitted", () => {
    const withNull = updateApplicationStatusSchema.parse({
      requestId: REQ,
      applicationId: APP,
      expectedVersion: 1,
      status: "APPLIED",
    ...
    const omitted = updateApplicationStatusSchema.parse({
      requestId: REQ,
      applicationId: APP,
      expectedVersion: 1,
      status: "APPLIED",
...
describe("updateApplicationDetailsSchema", () => {
  it("rejects an update with no editable fields", () => {
    expect(
      updateApplicationDetailsSchema.safeParse({
        requestId: REQ,
        applicationId: APP,
        expectedVersion: 1,
      }).success,
...
describe("searchApplicationsSchema", () => {
  it("bounds limit and validates enums", () => {
    ...
    expect(
      searchApplicationsSchema.safeParse({ text: "ibm", statuses: ["SAVED"], sort: "company" })
        .success,
    ).toBe(true);
  });
});
...
describe("candidateProfileSchema", () => {
  it("validates email and urls, blanks become null", () => {
    ...
    const parsed = candidateProfileSchema.parse({
      email: "",
      linkedinUrl: "https://linkedin.com/in/drew",
      phone: " ",
    });
