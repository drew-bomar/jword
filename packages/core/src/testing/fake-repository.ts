...
export class FakeTrackerRepository implements TrackerRepository {
  ...
  async searchApplications(userId: string, query: Required<Pick<SearchQuery, "limit">> & SearchQuery): Promise<Page<ApplicationOverview>> {
    ...
    if (text) rows = rows.filter((r) => r.company.toLowerCase().includes(text) || r.title.toLowerCase().includes(text));
    ...
    if (query.appliedFrom) rows = rows.filter((r) => r.appliedAt !== null && r.appliedAt >= query.appliedFrom!);
    if (query.appliedTo) rows = rows.filter((r) => r.appliedAt !== null && r.appliedAt <= query.appliedTo!);
    ...
    rows.sort((a, b) => {
      ...
      else if (sort === "company") cmp = a.company.localeCompare(b.company) || a.title.localeCompare(b.title);
  ...
  // ------------------------------------------------------------- internals
  private fingerprint(operation: string, ctx: MutationContext, command: Record<string, unknown>): string {
    const { requestId: _r, ...rest } = command;
    return `${operation}|${ctx.actor.actorType}|${JSON.stringify(rest, Object.keys(rest).sort())}`;
  }

  private beginRequest(ctx: MutationContext, requestId: string, operation: string, fp: string): MutationResult | null {
    const key = `${ctx.actor.userId}:${requestId}`;
    const receipt = this.receipts.get(key);
    if (!receipt) return null;
    if (receipt.operation !== operation || receipt.fingerprint !== fp) {
      throw new JwordError("CONFLICT", "This request id was already used for a different command.", {
  ...
  private finishRequest(ctx: MutationContext, requestId: string, operation: string, fp: string, result: MutationResult): MutationResult {
    this.receipts.set(`${ctx.actor.userId}:${requestId}`, { operation, fingerprint: fp, result });
    return { ...result, replayed: false };
  }

  private lockApplication(userId: string, applicationId: string, expectedVersion: number): ApplicationRecord {
    const app = this.applications.find((a) => a.userId === userId && a.id === applicationId);
    if (!app) throw new JwordError("NOT_FOUND", "Application not found.", { reason: "APPLICATION_NOT_FOUND" });
    if (app.version !== expectedVersion) {
      throw new JwordError("CONFLICT", "This application changed since you opened it. Refresh before saving.", {
  ...
  private resolveCompany(userId: string, name: string | null | undefined, companyId?: string): CompanyRecord {
    if (companyId) {
      const existing = this.companies.find((c) => c.userId === userId && c.id === companyId);
      if (!existing) throw new JwordError("NOT_FOUND", "Selected company was not found.", { reason: "COMPANY_NOT_FOUND" });
      return existing;
    }
    const clean = cleanText(name);
    if (!clean) throw new JwordError("VALIDATION_ERROR", "Company name is required.", { reason: "COMPANY_REQUIRED" });
    const normalized = normalizeName(clean);
    const existing = this.companies.find((c) => c.userId === userId && c.normalizedName === normalized);
  ...
  private addActivity(userId: string, applicationId: string, type: ActivityType, actorType: ActivityRecord["actorType"], summary: string, metadata: Record<string, unknown>, occurredAt?: string): ActivityRecord {
  ...
  private createTracked(
    ctx: MutationContext,
    command: Omit<CreateApplicationCommand, "requestId"> | (ImportRowCommand & { allowDuplicate?: boolean; initialNote?: string | null }),
    activityType: "CREATED" | "IMPORTED",
    metadata: Record<string, unknown>,
  ) {
    const userId = ctx.actor.userId;
    const title = cleanText(command.title);
    if (!title) throw new JwordError("VALIDATION_ERROR", "Job title is required.", { reason: "TITLE_REQUIRED" });
    const company = this.resolveCompany(userId, command.company, "companyId" in command ? command.companyId : undefined);
    ...
    if (!command.allowDuplicate) {
      const candidates: DuplicateCandidate[] = findDuplicateCandidates(
        { normalizedCompany: company.normalizedName, normalizedTitle: normalizeName(title), jobUrl, externalJobId },
        // synchronous version of listDuplicateIndex
        this.applications.filter((a) => a.userId === userId).map((a) => {
          const o = this.overview(a);
          return {
            applicationId: o.applicationId,
            company: o.company,
      ...
      if (candidates.length) {
        throw new JwordError("CONFLICT", "A similar application already exists. Confirm to create a separate application.", {
    ...
    const dateFound = "dateFound" in command && command.dateFound !== undefined ? command.dateFound : ctx.today;
    const appliedAt =
      "appliedAt" in command && command.appliedAt !== undefined ? command.appliedAt : status === "APPLIED" ? ctx.today : null;
    const ts = this.now();
    const job: JobRecord = {
      id: randomUUID(),
      userId,
    ...
    const noteBody = "initialNote" in command ? command.initialNote : "note" in command ? command.note : null;
    if (noteBody && noteBody.trim()) {
      noteId = randomUUID();
      this.notes.push({ noteId, userId, applicationId: app.id, body: noteBody, noteDate: null, createdAt: ts, updatedAt: ts });
    }
  ...
  // ------------------------------------------------------------- mutations
  async createApplication(ctx: MutationContext, command: CreateApplicationCommand): Promise<MutationResult> {
    ...
    return this.transaction(() => {
      ...
      return this.finishRequest(ctx, requestId, op, fp, {
        ...
        before: {},
        after: { status: app.status, priority: app.priority, dateFound: app.dateFound, appliedAt: app.appliedAt },
      });
    });
  }

  async updateApplicationStatus(ctx: MutationContext, command: UpdateApplicationStatusCommand): Promise<MutationResult> {
    const op = "update_application_status";
    const fp = this.fingerprint(op, ctx, command);
    const existing = this.beginRequest(ctx, command.requestId, op, fp);
    if (existing) return existing;
    return this.transaction(() => {
      const app = this.lockApplication(ctx.actor.userId, command.applicationId, command.expectedVersion);
      ...
      else if (statusChanged && command.status === "APPLIED" && app.appliedAt === null) newApplied = ctx.today;
      ...
      if (!statusChanged && !appliedChanged) {
        return this.finishRequest(ctx, command.requestId, op, fp, {
          ok: true, operation: op, requestId: command.requestId, replayed: false, noop: true,
          applicationId: app.id, version: app.version,
      ...
      if (statusChanged) { changed.push("status"); before.status = app.status; after.status = command.status; }
      if (appliedChanged) { changed.push("appliedAt"); before.appliedAt = app.appliedAt; after.appliedAt = newApplied; }
      const ts = this.now();
      ...
      app.updatedAt = ts;
      const summary = statusChanged ? `Status changed from ${before.status} to ${after.status}` : "Applied date updated";
      const activity = this.addActivity(
        ctx.actor.userId, app.id, statusChanged ? "STATUS_CHANGED" : "DETAILS_UPDATED", ctx.actor.actorType,
        summary, { fields: changed, before, after }, command.occurredAt,
      );
      return this.finishRequest(ctx, command.requestId, op, fp, {
        ok: true, operation: op, requestId: command.requestId, replayed: false, noop: false,
        applicationId: app.id, version: app.version, activityId: activity.activityId,
        summary, changedFields: changed, before, after,
      });
    });
  }

  async updateApplicationDetails(ctx: MutationContext, command: UpdateApplicationDetailsCommand): Promise<MutationResult> {
    ...
    return this.transaction(() => {
      ...
      const track = (field: string, oldValue: string | null, newValue: string | null) => {
        if (oldValue !== newValue) { changed.push(field); before[field] = oldValue; after[field] = newValue; return true; }
        return false;
      };
      const appPatch: Partial<ApplicationRecord> = {};
      const jobPatch: Partial<JobRecord> = {};
      if (command.priority !== undefined && track("priority", app.priority, command.priority)) appPatch.priority = command.priority;
      if (command.appliedAt !== undefined && track("appliedAt", app.appliedAt, command.appliedAt)) appPatch.appliedAt = command.appliedAt;
      if (command.dateFound !== undefined && track("dateFound", app.dateFound, command.dateFound)) appPatch.dateFound = command.dateFound;
      if (command.resumeVersion !== undefined && track("resumeVersion", app.resumeVersion, command.resumeVersion)) appPatch.resumeVersion = command.resumeVersion;
      if (command.referral !== undefined && track("referral", app.referral, command.referral)) appPatch.referral = command.referral;
      if (command.title !== undefined && track("title", job.title, command.title)) { jobPatch.title = command.title; jobPatch.normalizedTitle = normalizeName(command.title); }
      if (command.jobUrl !== undefined && track("jobUrl", job.jobUrl, command.jobUrl)) jobPatch.jobUrl = command.jobUrl;
      if (command.externalJobId !== undefined && track("externalJobId", job.externalJobId, command.externalJobId)) jobPatch.externalJobId = command.externalJobId;
      if (command.location !== undefined && track("location", job.location, command.location)) jobPatch.location = command.location;
      if (command.workArrangement !== undefined && track("workArrangement", job.workArrangement, command.workArrangement)) jobPatch.workArrangement = command.workArrangement;
      if (command.source !== undefined && track("source", job.source, command.source)) jobPatch.source = command.source;
      if (command.datePosted !== undefined && track("datePosted", job.datePosted, command.datePosted)) jobPatch.datePosted = command.datePosted;
      if (command.description !== undefined && job.description !== command.description) { changed.push("description"); jobPatch.description = command.description; }
      ...
      if (changed.length === 0) {
        return this.finishRequest(ctx, command.requestId, op, fp, {
          ok: true, operation: op, requestId: command.requestId, replayed: false, noop: true,
          applicationId: app.id, version: app.version, summary: "No changes were needed.",
          changedFields: [], before: {}, after: {},
        });
      }
      const ts = this.now();
      ...
      const activity = this.addActivity(userId, app.id, "DETAILS_UPDATED", ctx.actor.actorType, summary, { fields: changed, before, after });
      return this.finishRequest(ctx, command.requestId, op, fp, {
        ok: true, operation: op, requestId: command.requestId, replayed: false, noop: false,
        applicationId: app.id, version: app.version, activityId: activity.activityId,
        summary, changedFields: changed, before, after,
  ...
  async addApplicationNote(ctx: MutationContext, command: AddApplicationNoteCommand): Promise<MutationResult> {
    ...
    return this.transaction(() => {
      ...
      this.notes.push({ noteId, userId: ctx.actor.userId, applicationId: app.id, body: command.note, noteDate: command.noteDate ?? null, createdAt: ts, updatedAt: ts });
      app.version += 1;
      app.lastActivityAt = ts;
      app.updatedAt = ts;
      const activity = this.addActivity(ctx.actor.userId, app.id, "NOTE_ADDED", ctx.actor.actorType, "Added a note", { noteId, noteDate: command.noteDate ?? null });
      return this.finishRequest(ctx, command.requestId, op, fp, {
        ok: true, operation: op, requestId: command.requestId, replayed: false, noop: false,
        applicationId: app.id, noteId, version: app.version, activityId: activity.activityId,
        summary: "Note added", changedFields: ["note"], before: {}, after: { noteDate: command.noteDate ?? null },
      });
    });
  }

  async updateApplicationNote(ctx: MutationContext, command: UpdateApplicationNoteCommand): Promise<MutationResult> {
    ...
    return this.transaction(() => {
      const app = this.lockApplication(ctx.actor.userId, command.applicationId, command.expectedVersion);
      const note = this.notes.find((n) => n.userId === ctx.actor.userId && n.noteId === command.noteId && n.applicationId === app.id);
      if (!note) throw new JwordError("NOT_FOUND", "Note not found on this application.", { reason: "NOTE_NOT_FOUND" });
      const changed: string[] = [];
      ...
      if (changed.length === 0) {
        return this.finishRequest(ctx, command.requestId, op, fp, {
          ok: true, operation: op, requestId: command.requestId, replayed: false, noop: true,
          applicationId: app.id, noteId: note.noteId, version: app.version, summary: "Note is already up to date.",
          changedFields: [], before: {}, after: {},
        });
      }
      const ts = this.now();
      const metadata = { noteId: note.noteId, fields: changed, before: { note: note.body, noteDate: note.noteDate }, after: { note: body, noteDate } };
      note.body = body;
      ...
      app.lastActivityAt = ts;
      app.updatedAt = ts;
      const activity = this.addActivity(ctx.actor.userId, app.id, "NOTE_UPDATED", ctx.actor.actorType, `Updated a note (${changed.join(", ")})`, metadata);
      return this.finishRequest(ctx, command.requestId, op, fp, {
        ok: true, operation: op, requestId: command.requestId, replayed: false, noop: false,
        applicationId: app.id, noteId: note.noteId, version: app.version, activityId: activity.activityId,
        summary: "Note updated", changedFields: changed, before: {},
  ...
  async importApplications(ctx: MutationContext, command: CommitImportCommand): Promise<MutationResult> {
    ...
    return this.transaction(() => {
      ...
      for (const row of command.rows) {
        try {
          ...
          const { app } = this.createTracked(
            ...
            { ...rest, rowIndex, dateFound: row.dateFound ?? null, appliedAt: row.appliedAt ?? null, allowDuplicate: duplicateChoice === "import_separate" },
            "IMPORTED",
            { rowIndex },
          );
          ids.push(app.id);
          ...
          if (error instanceof JwordError && error.code === "CONFLICT") {
            throw new JwordError("IMPORT_ROW_ERROR", `Row ${row.rowIndex} matches an existing application and has no duplicate choice.`, { reason: "DUPLICATE_UNRESOLVED", rowIndex: row.rowIndex });
          }
          if (error instanceof JwordError && (error.code === "VALIDATION_ERROR" || error.code === "NOT_FOUND")) {
            throw new JwordError("IMPORT_ROW_ERROR", `Row ${row.rowIndex}: ${error.message}`, { reason: "ROW_INVALID", rowIndex: row.rowIndex });
          }
          throw error;
        }
      }
      return this.finishRequest(ctx, command.requestId, op, fp, {
        ok: true, operation: op, requestId: command.requestId, replayed: false, noop: false,
        imported: ids.length, applicationIds: ids, summary: `Imported ${ids.length} application(s)`,
        changedFields: [], before: {}, after: {},
      });
    });
  }
